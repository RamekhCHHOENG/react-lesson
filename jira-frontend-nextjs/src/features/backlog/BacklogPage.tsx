...
import type { Task, TaskStatus, IssuePriority, IssueType } from "@/types/project"
...
function TaskRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_120px_140px_100px_110px_110px_80px] gap-3 px-4 py-2.5 border-b hover:bg-accent/50 transition-colors group items-center">
      ...
      <div>
        <span className="text-xs text-muted-foreground truncate block">{task.reporter || "System"}</span>
      </div>
      <div>
        {task.assignee ? (
...
export default function BacklogPage() {
  ...
  const filteredTasks = useMemo(() => {
    return allTasks.filter((t) => {
      ...
      const matchType = issueTypeFilter === "all" || t.issueType === issueTypeFilter
  ...
  }, [allTasks, search, statusFilter, issueTypeFilter, priorityFilter, selectedProjectId])
              ...
              <Card key={key} className="overflow-hidden">
                ...
                {expandedSections[key] && (
                  <CardContent className="p-0">
                    {/* Column headers */}
                    <div className="grid grid-cols-[1fr_120px_140px_100px_110px_110px_80px] gap-3 px-4 py-1.5 bg-muted/30 border-b text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                      <span>Issue</span>
                      <span>Project</span>

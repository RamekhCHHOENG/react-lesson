...
function buildTempIssueKey(project: Project): string {
  return `${project.key || "PRJ"}-TMP-${project.tasks.length + 1}`
}
...
export function useTasks(projectId: string) {
  ...
  const addMutation = useMutation({
    ...
    onMutate: async (data) => {
      await qc.cancelQueries({ queryKey: PROJECTS_KEY })
      const previous = readCache(qc)
      const project = previous.find((item) => item.id === projectId)
      if (!project) return { previous }

      const tempTask = {
        id: `temp-${Date.now()}`,
        issueKey: buildTempIssueKey(project),
        ...data,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

...
import { APP_CONFIG } from "@/config/project"
...
function addTask(
  projectId: string,
  task: Omit<import("@/types/project").Task, "id" | "createdAt" | "updatedAt">
): import("@/types/project").Task | undefined {
  const all = getAll()
  const index = all.findIndex((p) => p.id === projectId)
  if (index === -1) return undefined
  const now = new Date().toISOString()
  const newTask: import("@/types/project").Task = {
    ...task,
    id: generateId(),
    createdAt: now,
    updatedAt: now,
...
function updateTask(
  projectId: string,
  taskId: string,
  data: Partial<Omit<import("@/types/project").Task, "id" | "createdAt">>
): import("@/types/project").Task | undefined {
  const all = getAll()
